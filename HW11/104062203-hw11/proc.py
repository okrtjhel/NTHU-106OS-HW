...
class ProcessFS:
	...
	def open(self, filepath):
		...
		obj = enclosingDir.lookup(filename)
		if obj is None or (isinstance(obj, FCB) is False):
			raise ValueError('%s is not a file or does not exist' % filename)
		else:
			# if the FCB is not already in the system-wide open file table,
			# then add it, and increment its open count.
			if obj not in self.fs.sysOpenFileTable:
				self.fs.sysOpenFileTable.append(obj)
			obj.incrOpenCount()
			...
			ppf = PerProcessFileEntry(obj)
			self.openFileTable.append(ppf)
			descriptor = self.openFileTable.index(ppf)
			ppf.fcb.updateAccessTime()
			return descriptor
	...
	def close(self, descriptor):
		...
		ppf = self.openFileTable[descriptor]
		fcb = ppf.fcb
		fcb.decrOpenCount()
		if fcb.openCount is 0:
			del self.sysOpenFileTable[self.sysOpenFileTable.index(fcb)]
		del self.openFileTable[descriptor]
	...
	def read(self, descriptor, nBlocks=1):
		...
		for i in range(nBlocks):
			data = data + ppf.fcb.index[ppf.pos]
			num = num + 1
			if ppf.pos is len(ppf.fcb.index)-1:
				break
			ppf.pos = ppf.pos + 1
		ppf.fcb.updateAccessTime()
		return data
	...
	def write(self, descriptor, data):
		...
		ppf = self.openFileTable[descriptor]
		newBlock = self.fs.allocateBlocks(len(data))
		if newBlock is None:
			raise ValueError('Have no enough space')
		else:
			ppf.fcb.index.extend(newBlock)
			for i in range(len(data)):
				ppf.fcb.index[ppf.pos] = data[i]
				ppf.pos = ppf.pos + 1
			ppf.fcb.updateModTime()
